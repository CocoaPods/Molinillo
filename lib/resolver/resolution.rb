  ...
  class Resolver
    class Resolution
      ...
      def unwind_for_conflict
        debug { 'Unwinding from level ' + state.depth.to_s }
        states.pop
      end
      ...
      def debug(&block)
        resolver_ui.debug(&block)
      end

      def attempt_to_activate(requested_spec)
        debug { 'attempting to activate ' + name + ' at ' + requested_spec.to_s }
      ...
      def attempt_to_activate_new_spec(requested_spec)
        satisfied = begin
          ...
          debug { 'Unsatisfied by requested spec' } unless requested_spec_satisfied
          debug { 'Unsatisfied by locked spec' } unless locked_spec_satisfied
      ...
      def activate_spec(spec_to_activate)
        debug { 'activated ' + name_for(spec_to_activate) + ' at ' + spec_to_activate.to_s }
      ...
      def require_nested_dependencies_for(activated_spec)
        debug { 'requiring nested dependencies' }
